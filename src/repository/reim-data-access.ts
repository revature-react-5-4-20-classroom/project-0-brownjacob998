  ...
  try {
    ...
    result = await client.query(
        ...
        reimbursement.resolver, status.status, "Type"."type"
         ...
         reimbursement."type" = "Type".typeID AND reimbursement.status = $1 AND status.statusID = reimbursement.status ORDER BY reimbursement.dateSubmitted DESC;`,[id]
    );
    console.log(result)
    ...
    try {
        ...
        result = await client.query(
            ...
            reimbursement.resolver, status.status, "Type"."type"
             ...
             reimbursement."type" = "Type".typeID AND status.statusID = reimbursement.status ORDER BY reimbursement.dateSubmitted DESC;`, [id]
        );
        return result.rows.map((r) => {
        return new Reimbursement(r.reimbursementid, r.name, r.amount, r.datesubmitted, r.dateresolved, r.description, r.resolver, r.status, r.type);
        });
